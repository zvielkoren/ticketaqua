...
Guild = client.get_guildS

my_secret = os.environ['token']

TicketEmbedButtonName = "פתיחת טיקט"  #after the message of the open tickets has been sent, this line can be deleted
TicketEmbedButtonEmoji = "🎫"  #  a  fter the message of the open tickets has been sent, this line can be deleted
...
@client.command(
)  #after the message of the open tickets has been sent, this line can be deleted
async def send_ticket_command(
    ctx
):  #after the message of the open tickets has been sent, this line can be deleted
    ...
    await channel.send(
        embed=embed,
        components=[
            Button(label=TicketEmbedButtonName,
                   emoji=TicketEmbedButtonEmoji,
...
async def on_button_click(button):
    ...
    if button.component.label == TicketEmbedButtonName:
        ...
        if not varexists(f"{user.id}ticket", "tickets"):
            ...
            embed = discord.Embed(
                title=f"{guild.name} TICKETS",
                description=
                f"הצוות יגיע בקרוב.\n כדי לסגור את הטיקט, כתבו ``-close``",
                color=discord.Colour.blue())
...
async def close(ctx):
    if varexists(f"{ctx.channel.id}creator", "tickets"):
        embed = discord.Embed(description=f"הטיקט ייסגר בקרוב",
                              colour=discord.Colour.dark_red())
        await ctx.reply(embed=embed)
        user = get(client.get_all_members(),
                   id=int(getvar(f"{ctx.channel.id}creator", "tickets")))
        messages = await ctx.channel.history(limit=150000,
